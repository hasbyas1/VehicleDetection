...
void updateOLED() {
    ...
    // Line 4: Controller IP
    display.setCursor(0, 37);
    display.print("Ctrl: ");
    display.println(controllerIP);
    
    // Line 5: Barrier Status
...
void showWiFiStatus(String ssid, int attempt, String status) {
    display.clearDisplay();
    display.setTextSize(1);
    display.setTextColor(SSD1306_WHITE);
    display.setCursor(0, 0); 
...
void connectWiFiMulti() {
    ...
    showWiFiStatus(ssid_primary, 1, "Connecting...");  // Print on OLED
    ...
    while (WiFi.status() != WL_CONNECTED && retry < WIFI_MAX_RETRY) {
        ...
        Serial.print(".");
        retry++;
        showWiFiStatus(ssid_primary, 1, "Retrying " + String(retry) + "/" + String(WIFI_MAX_RETRY));  // Print on OLED
    }
    ...
    if (WiFi.status() != WL_CONNECTED) {
        ...
        showWiFiStatus(ssid_primary, 1, "FAILED!");  // Print on OLED
        delay(1000);                                 // Delay (so user can read it)
        WiFi.disconnect();
        delay(1000);
        
        Serial.print("Trying WiFi 2: ");
        Serial.println(ssid_backup1);
        showWiFiStatus(ssid_backup1, 2, "Connecting..."); // Print on OLED
        ...
        while (WiFi.status() != WL_CONNECTED && retry < WIFI_MAX_RETRY) {
            ...
            Serial.print(".");
            retry++;
            showWiFiStatus(ssid_backup1, 2, "Retrying " + String(retry) + "/" + String(WIFI_MAX_RETRY)); // Print on OLED
        }
    }
    ...
    if (WiFi.status() != WL_CONNECTED) {
        ...
        showWiFiStatus(ssid_backup1, 2, "FAILED!");     // Print on OLED 
        delay(1000);                                    // Delay (so user can read it)
        WiFi.disconnect();
        delay(1000);
        ...
        showWiFiStatus(ssid_backup2, 3, "Connecting...");
        ...
        while (WiFi.status() != WL_CONNECTED && retry < WIFI_MAX_RETRY) {
            ...
            Serial.print(".");
            retry++;
            showWiFiStatus(ssid_backup2, 3, "Retrying " + String(retry) + "/" + String(WIFI_MAX_RETRY));
        }
    }
    ...
    if (WiFi.status() == WL_CONNECTED) {
        ...
        display.setCursor(0, 20); display.println("All networks tried.");
        display.setCursor(0, 35); display.println("Restarting in 5s...");
        display.display();
